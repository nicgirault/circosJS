...
export function parseSpanValueData(data, layoutSummary) {
  ...
  const filteredData = preParsedData
    .filter((datum, index) => checkParent(datum[0], index, layoutSummary, 'parent'))
    .filter((datum, index) => checkNumber({start: datum[1], end: datum[2], value: datum[3]}, index))
    .map((datum) => {
...
export function parseSpanStringData(data, layoutSummary) {
  ...
  const filteredData = preParsedData
    .filter((datum, index) => checkParent(datum[0], index, layoutSummary, 'parent'))
    .filter((datum, index) => checkNumber({start: datum[1], end: datum[2]}, index))
    .map((datum) => {
      if (datum[1] < 0 || datum[2] > layoutSummary[datum[0]]) {
        logger.log(
          2,
...
export function parsePositionValueData(data, layoutSummary) {
  ...
  const filteredData = preParsedData
    .filter((datum, index) => checkParent(datum[0], index, layoutSummary, 'parent'))
    .filter((datum, index) => checkNumber({position: datum[1], value: datum[2]}, index))
    .map((datum) => {
      return {
        block_id: datum[0],
...
export function parsePositionTextData(data, layoutSummary) {
  ...
  const filteredData = data
    .filter((datum, index) => checkParent(datum[0], index, layoutSummary, 'parent'))
    .filter((datum, index) => checkNumber({position: datum[1]}, index))
    .map((datum) => {
      return {
        block_id: datum[0],
        position: Math.min(layoutSummary[datum[0]], parseFloat(datum[1])),
...
export function parseChordData(data, layoutSummary) {
  ...
  .filter((datum, index) => checkParent(datum[0], index, layoutSummary, 'source_id'))
  .filter((datum, index) => checkParent(datum[3], index, layoutSummary, 'target_id'))
